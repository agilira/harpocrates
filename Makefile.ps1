...
function Invoke-Help {
    ...
    Write-ColorOutput "  fuzz          Run fuzz tests for security validation" $Green
...
function Invoke-Fuzz {
    ...
    Write-ColorOutput "Testing DecryptBytes with random inputs..." $Blue
    go test -fuzz=FuzzDecryptBytes -fuzztime=10s
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    
    Write-ColorOutput "Testing round-trip encryption/decryption..." $Blue
    go test -fuzz=FuzzEncryptDecryptRoundTrip -fuzztime=10s
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    
    Write-ColorOutput "Fuzz tests completed successfully!" $Green
}
